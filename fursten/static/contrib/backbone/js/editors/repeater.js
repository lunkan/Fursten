;(function(Form) {
  ...
  Form.editors.Repeater = Form.editors.Base.extend({
    ...
    createHeaders: function() {
	  ...
	  var schemaColgroups = this.schema.colgroups || {};
	  ...
	  if(this.schema.subSchema) {
		_.each(this.schema.subSchema, function(value, key) {
			...
			var headerColwidth = schemaColgroups[key] || "";
			var header = { title: headerText, help:value.help, colwidth:headerColwidth };
		    headers.push(header);
	    });
  ...
  Form.editors.Repeater.Item = Form.editors.Base.extend({
    ...
    removeButtonTemplate: _.template('\
  	  <td><button type="button" data-target="<%= itemId %>" data-action="remove" class="btn btn-warning pull-right">&times;</button></td>\
  ...
  Form.editors.Repeater.Form = Form.extend({
	  ...
	  fieldTemplate: _.template('\
	    <td class="h-layout form-group">\
	      <div class="repeater-field" data-editor></div>\
